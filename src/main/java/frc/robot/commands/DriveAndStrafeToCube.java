...
public class DriveAndStrafeToCube extends CommandBase {
  ...
  public DriveAndStrafeToCube(SwerveSubsystem drivetrain, CubeVisionSubsystem vision,
  ...
  public void execute() {
    ...
    if (vision.hasTargets()) {
      ...
      if (controller.atSetpoint()) {
        xSpeed = Math.hypot(xSpeed, ySpeed);
        drivetrain.drive(xSpeed, 0, rotationalSpeed, false);
        return;
      }
    } else {
      rotationalSpeed = -driveController.getRightX();
      rotationalSpeed = MathUtil.applyDeadband(rotationalSpeed, DEADBAND) * inputScalar;
    }
